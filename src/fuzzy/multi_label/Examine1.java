...
//		String dataName = "yeast_multi";
		...
		//対象の結論部を持つルールを削除したルール集合を作成する
		int[][] existLabels = readExistLabels(dataName);
		MultiRuleSet[] ruleSets = new MultiRuleSet[existLabels.length];
		for(int i = 0; i < existLabels.length; i++) {
			ruleSets[i] = removeRulesByLabel(classifier, existLabels[i]);
		}

		//削除した結論部を正しく識別できるか?
		//(= ルール集合に存在しない結論部の出力が可能か？)
		for(int i = 0; i < existLabels.length; i++) {
			...
			for(int p = 0; p < dataSize; p++) {
				...
				int[] answer = ruleSets[i].cfVectorClassify(pattern, false);

				if(Arrays.equals(label, answer)) {
					System.out.println(p);
					correct++;
					ruleSets[i].cfVectorClassify(pattern, false);
				}
			...
			System.out.println();
			System.out.println("get unknow count: " + correct);
...
//		Output.writeln(fileName, originalClassifier);
	...
	public static MultiRuleSet removeRulesByLabel(MultiRuleSet ruleSet, int[] label) {
		MultiRuleSet newRuleSet = new MultiRuleSet();

		for(int r = 0; r < ruleSet.getRuleNum(); r++) {
			MultiRule rule = ruleSet.getMicRule(r);	//Shallow Copy

			if(!Arrays.equals(rule.getConc(), label)) {
				newRuleSet.addRule(rule);	//Deep Copy
			}
		}

		newRuleSet.calcRuleLength();

		return newRuleSet;
	}

	public static int[][] readExistLabels(String dataName) {
		String sep = File.separator;
		String fileName = "multi-label_real" + sep + dataName + "_existLabels.csv";
		List<String> lines = Input.inputAsListString(fileName);

		int[][] existLabels = new int[lines.size()][];
		for(int i = 0; i < lines.size(); i++) {
			String[] line = lines.get(i).split(",");
			int[] label = new int[line.length];
			for(int c = 0; c < label.length; c++) {
				label[c] = Integer.parseInt(line[c]);
			}
			existLabels[i] = label;
		}

...
public class Output_MultiLabel extends OutputClass<MultiPittsburgh>{
	...
	public String outputPredicted(Population<MultiPittsburgh> population, MultiDataSetInfo dataset) {
		...
		for(int i = 0; i < dataset.getDataSize(); i++) {
			MultiPattern pattern = dataset.getPattern(i);
			str = array2str(pattern.getConClass());
			for(int p = 0; p < population.getIndividuals().size(); p++) {
				str += c + array2str(population.getIndividual(p).getRuleSet().classifyParallel(pattern, false));
			}
			strs += str + ln;
		}
		return strs;
	...
	public String array2str(int[] array) {
		String str = "";
